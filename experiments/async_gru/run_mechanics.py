    ...
    return [
        {
            ...
            "latest_version_id": tick_trace.latest_version_id,
            "read_version_ids": list(tick_trace.read_version_ids),
            "max_read_vs_latest_abs_diff": [
                round(float(value), 6) for value in tick_trace.max_read_vs_latest_abs_diff
            ],
        }
...
def main() -> None:
    ...
    output_dir = args.output_dir or (args.repo_root / "experiments" / "async_gru" / "artifacts")
    ...
    report = {
        ...
        "checks": {
            ...
            "zero_staleness_equivalence": zero_staleness_equivalence(
                sync_trace,
                zero_trace,
                sync_logits,
                zero_logits,
            ...
            "async_vs_sync_output_delta": {
                "max_history_abs_diff": round(
                    float(max_history_abs_diff(sync_trace.history, async_trace.history)),
                    6,
                ),
    ...
    output_path = output_dir / "mechanics_report.json"
    write_json(output_path, report)
    print("PASS async_gru mechanics")
