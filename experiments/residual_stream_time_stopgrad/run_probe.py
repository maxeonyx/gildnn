...
from .model import (
    ResidualRunTrace,
    ResidualStreamTimeStopGradCharModel,
    ResidualStreamTimeStopGradConfig,
    count_parameters,
)
...
def trace_to_payload(trace: ResidualRunTrace) -> dict[str, object]:
    return {
        ...
        "mix_coefficients": trace.mix_coefficients,
...
def capture_sample(
    model: ResidualStreamTimeStopGradCharModel,
...
def write_metadata(
    ...
    *,
    ...
    write_json(
        ...
        {
            "model_family": "residual_stream_time_stopgrad",
            "experiment_parent": "residual_stream_time_mixadd",
            ...
            "stop_gradient_across_time": True,
        },
    )
...
def main() -> None:
    ...
    output_dir = args.output_dir or (
        args.repo_root / "experiments" / "residual_stream_time_stopgrad" / "artifacts" / args.stage
    )
    ...
    model = ResidualStreamTimeStopGradCharModel(
        vocab_size=split.train_dataset.vocab_size,
        config=ResidualStreamTimeStopGradConfig(
            context_size=config.context_size,
            d_model=config.d_model,
            feedforward_dim=config.feedforward_dim,
            temporal_window=config.temporal_window,
    ...
    if args.stage == "overfit":
        ...
        write_loss_curve_svg(
            ...
            title="Residual stream across time stop-grad: overfit one batch",
    ...
    write_loss_curve_svg(
        ...
        title="Residual stream across time stop-grad: tiny run",
        series={
