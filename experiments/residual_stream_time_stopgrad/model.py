...
@dataclass(frozen=True)
class ResidualStreamTimeStopGradConfig:
    context_size: int
    d_model: int
    feedforward_dim: int
...
class ResidualStreamTimeStopGradCharModel(nn.Module):
    def __init__(self, *, vocab_size: int, config: ResidualStreamTimeStopGradConfig) -> None:
        super().__init__()
        self.config = config
    ...
    def _run(self, tokens: Tensor, *, capture_trace: bool) -> tuple[Tensor, ResidualRunTrace | None]:
        ...
        for time_index in range(self.context_size):
            previous_stream = stream.detach()
            block_input = self.mix_token(previous_stream, embeddings[:, time_index, :])
