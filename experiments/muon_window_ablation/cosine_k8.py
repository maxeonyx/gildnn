"""Muon continuation: k=8 with cosine LR annealing for 15 epochs.

Flat Muon LR=0.01 diverged at epoch 10 in the 15-epoch extension, with
best val 1.6613 at epoch 6. This run keeps the same k=8 setup but anneals
the Muon and AdamW learning rates with a cosine schedule.

Run: python -m experiments.muon_window_ablation.cosine_k8
"""
...
OUTPUT_FILE = ARTIFACTS_DIR / "muon_k8_cosine_15ep.json"
...
FLAT_LR_BEST_REFERENCE = 1.6613
FLAT_LR_BEST_EPOCH = 6
...
def main():
    ...
    print(
        f"\nStarting k={TEMPORAL_WINDOW} with cosine Muon LR={MUON_LR} for {EPOCHS} epochs",
        flush=True,
    )
    result = train_run(
        device=device,
    ...
    print(f"Muon k=8 cosine, 15ep: best_val={status}", flush=True)
    print(f"Transformer reference: {TRANSFORMER_REFERENCE:.3f}", flush=True)
    print(f"Flat LR best: {FLAT_LR_BEST_REFERENCE:.4f} at epoch {FLAT_LR_BEST_EPOCH}", flush=True)
    ...
    if not result["diverged"]:
        ...
        print(
            f"Delta vs flat LR best: {result['best_val'] - FLAT_LR_BEST_REFERENCE:+.4f}",
            flush=True,
        )
        print(
    ...
    payload = {
        ...
        "references": {
            ...
            "flat_lr_best": {
                "val": FLAT_LR_BEST_REFERENCE,
                "epoch": FLAT_LR_BEST_EPOCH,
            },
