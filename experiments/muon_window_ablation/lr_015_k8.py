"""Muon feasibility check: k=8 at Muon LR=0.015, 5 epochs.

Phase 2 showed k=8 reached val 1.664 after 5 epochs at d=192 with Muon LR=0.01.
This run keeps the same setup and checks whether LR=0.015 stays stable over 5 epochs.

Run: python -m experiments.muon_window_ablation.lr_015_k8
"""
...
OUTPUT_FILE = ARTIFACTS_DIR / "muon_k8_lr015.json"
...
MUON_LR_001_5EP_REFERENCE = 1.664
...
def main():
    ...
    print(f"Muon k=8, 5ep: best_val={status}", flush=True)
    print(f"Transformer reference: {TRANSFORMER_REFERENCE:.3f}", flush=True)
    print(f"Muon LR=0.01, k=8, 5ep reference: {MUON_LR_001_5EP_REFERENCE:.3f}", flush=True)
    if not result["diverged"]:
        ...
        print(
            f"Delta vs Muon LR=0.01, 5ep: {result['best_val'] - MUON_LR_001_5EP_REFERENCE:+.4f}",
            flush=True,
        )
    ...
    payload = {
        ...
        "references": {
            ...
            "muon_lr_0.01_k8_5ep": MUON_LR_001_5EP_REFERENCE,
        },
        "result": result,
    }
