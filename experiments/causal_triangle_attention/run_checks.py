        ...
        block_rows.append(
            {
                ...
                "memory_cell_count": trace.memory_cell_count,
...
def gradient_check(
    ...
    if model.variant.family != "external_control":
        if not any(float(block_row["depth_path_grad_norm"]) > 0.0 for block_row in block_rows):
            raise RuntimeError(f"{model.variant.family} depth path gradients were zero in every block.")
    return {
        "loss": loss.item(),
...
def depth_memory_access_check(
    ...
    *,
    ...
    if model.variant.family == "causal_triangle":
        return {"note": "See causal_triangle_mask_probe for the 2D memory probe."}
    run = model.run(tokens, capture_details=True)
    ...
    for trace in run.block_traces:
        ...
        base_update, _ = block.depth_readout(
            current_state,
        ...
        for slot_index, _memory_state in enumerate(boundary_memory):
            ...
            other_token_update, _ = block.depth_readout(
                current_state,
        ...
        if trace.depth_attention_weights is not None:
            block_row["attention_preview"] = trace.depth_attention_weights[0, 0, :, :].tolist()
        rows.append(block_row)
    return {"blocks": rows}


def causal_triangle_mask_probe(
    model: AttentionResidualCharModel,
    tokens: Tensor,
    *,
    perturbation_scale: float,
    ...
    if model.variant.family != "causal_triangle":
        return {"note": "Only applies to the causal_triangle variant."}
    run = model.run(tokens, capture_details=True)
    block_index = 1
    query_position = 3
    block = model.blocks[block_index]
    current_state = run.boundary_states[block_index]
    boundary_memory = [state.clone() for state in run.boundary_states[: block_index + 1]]
    base_update, weights = block.depth_readout(
        current_state,
        boundary_memory=boundary_memory,
        capture_details=True,
    )
    if weights is None:
        raise RuntimeError("Expected causal_triangle depth readout to return attention weights.")

    def measure_change(layer_index: int, position_index: int) -> float:
        edited_memory = [state.clone() for state in boundary_memory]
        edited_memory[layer_index][:, position_index, 0] += perturbation_scale
        edited_update, _ = block.depth_readout(
            current_state,
            boundary_memory=edited_memory,
            capture_details=False,
        )
        return (edited_update[:, query_position, :] - base_update[:, query_position, :]).abs().max().item()

    probes = {
        "allowed_same_layer_past_timestep": {
            "layer_index": block_index,
            "position_index": query_position - 1,
        },
        "allowed_earlier_layer_same_timestep": {
            "layer_index": 0,
            "position_index": query_position,
        },
        "forbidden_same_layer_same_timestep": {
            "layer_index": block_index,
            "position_index": query_position,
        },
        "forbidden_same_layer_future_timestep": {
            "layer_index": block_index,
            "position_index": query_position + 1,
        },
        "forbidden_earlier_layer_future_timestep": {
            "layer_index": 0,
            "position_index": query_position + 1,
        },
    }
    ...
    for name, probe in probes.items():
        measured[name] = {
            **probe,
            "max_abs_change_at_query": measure_change(
                layer_index=int(probe["layer_index"]),
                position_index=int(probe["position_index"]),
            ),
        }

    if measured["allowed_same_layer_past_timestep"]["max_abs_change_at_query"] <= 1e-8:
        raise RuntimeError("Allowed same-layer past-timestep probe did not affect the query output.")
    if measured["allowed_earlier_layer_same_timestep"]["max_abs_change_at_query"] <= 1e-8:
        raise RuntimeError("Allowed earlier-layer same-timestep probe did not affect the query output.")
    for forbidden_name in (
        "forbidden_same_layer_same_timestep",
        "forbidden_same_layer_future_timestep",
        "forbidden_earlier_layer_future_timestep",
    ):
        if measured[forbidden_name]["max_abs_change_at_query"] > 1e-8:
            raise RuntimeError(
                f"Forbidden probe {forbidden_name} changed the query output by "
                f"{measured[forbidden_name]['max_abs_change_at_query']:.12f}."
            )

    preview = weights[0, query_position, :, :, :].tolist()
    attn_mask = CausalTriangleReadout.attention_mask(
        sequence_length=tokens.shape[1],
        memory_layers=block_index + 1,
        current_layer_index=block_index,
        device=tokens.device,
    )
    return {
        "block_index": block_index + 1,
        "query_position": query_position,
        "attention_preview": preview,
        "mask_preview": attn_mask[query_position].reshape(block_index + 1, tokens.shape[1]).tolist(),
        "probes": measured,
    }
...
def rough_compute_table(config: RunConfig) -> list[dict[str, object]]:
    ...
    for variant in VARIANTS.values():
        ...
        for block_index in range(variant.num_layers):
            ...
            if variant.family == "causal_triangle":
                depth_units = 4 * sequence_length * d_model * d_model + 2 * sequence_length * sequence_length * (block_index + 1) * d_model
...
def run_variant_checks(
    *,
    ...
    if variant.family == "causal_triangle":
        payload["causal_triangle_mask_probe"] = causal_triangle_mask_probe(
            model,
            tokens,
            perturbation_scale=config.perturbation_scale,
        )
...
def main() -> None:
    ...
    output_dir = args.output_dir or (
        args.repo_root / "experiments" / "causal_triangle_attention" / "artifacts" / "stage1_mask_probe"
    )
