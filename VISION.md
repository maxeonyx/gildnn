...
**Closed-loop hierarchical prediction — mechanism works, target bottleneck broken.** Block 1 predicts block 0's states and feeds predictions back via additive zero-init gate. The full locality series is complete: strict-local with ungrounded target collapses (D); strict-local with task-grounded local CE is stable but doesn't help (E, +0.026); semi-local (CE through interface) helps (C, -0.006). **The feedback gradient does two jobs: prevents collapse AND teaches what to predict. Local CE can replace the first but not the second.** N=3 star topology with full-state target saturates — width doesn't help because predicting full block-0 state is redundant.

**Phase 5 J CONFIRMED (2026-05-24): predicting older-window (mean of h_{t-8}..h_{t-5}) breaks the -0.006 ceiling.** Result: Δ = -0.010 with 53× less seed variance than C. The target was the bottleneck. The win is seed robustness (std 0.00009 vs 0.005 for C), not raw magnitude. Current work: testing temporal offset sensitivity (J_far, offset 9-12 — results pending). See `research/questions/local-learning-variants/README.md`.
...
- **Can local learning scale?** Partially answered. Strict-local: NO (collapses with ungrounded target; stable but harmful with task-grounded local CE). Neighborhood-local (CE flows through interface): YES at N=2. **Phase 5 J broke the -0.006 ceiling** — predicting older-window gives -0.010 with extreme reliability. Width scaling path REOPENED: multiple helpers with different temporal offsets (role differentiation) is the next topology experiment. Rate-4 predictions intrinsically too stale with current-state target; may work with older-window target (untested). Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
