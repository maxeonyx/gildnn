...
**Closed-loop hierarchical prediction — mechanism works, neighborhood-local confirmed as minimum viable locality.** Block 1 predicts block 0's future states and feeds predictions back via additive zero-init gate. V3 result (2 seeds): C < A by 0.006, gain spontaneously goes negative (predictive coding emerges). The full locality series is complete: strict-local with ungrounded target collapses (D); strict-local with task-grounded local CE is stable but doesn't help (E, +0.026); semi-local (CE through interface) helps (C, -0.006). **The feedback gradient does two jobs: prevents collapse AND teaches what to predict. Local CE can replace the first but not the second.** N=3 star topology (two helpers both predict block 0) is **seed-sensitive** — creates 15× more variance than baseline, one seed hurts (+0.034) one helps (-0.013). Rate-4 helper always rejected (gain → 0). Currently testing whether rate-4 is intrinsically viable in isolation (G_rate4_only). See `research/questions/local-learning-variants/README.md`.
