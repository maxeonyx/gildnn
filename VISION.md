...
**Closed-loop hierarchical prediction — mechanism works, strict-local fails.** Block 1 predicts block 0's future states and feeds predictions back into block 0's computation. V3 result (2 seeds): C < A by 0.006, gain spontaneously goes negative (predictive coding emerges). The mechanism is real — but only under semi-local (neighborhood-local) conditions where CE flows through the feedback interface. **Strict-local (zero cross-boundary gradient) collapses catastrophically** (D=3.05 vs A=1.67) — full-state cosine prediction doesn't select for task-relevant components. Currently testing whether a task-grounded local objective (local CE for block 1) rescues strict-local. See `research/questions/hierarchical-prediction/README.md` and `research/questions/local-learning-variants/README.md`.
...
- **Can local learning scale?** Partially answered. Full-state cosine prediction under strict-local gradients: NO — collapses every time (target isn't task-grounded). Neighborhood-local (CE flows through block interface): YES at N=2, effect small. Currently testing task-grounded strict-local (E_grounded). If that works, the next question is scaling to N>2. Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
