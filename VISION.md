...
- **Can local learning scale?** Partially answered. Strict-local: NO (collapses with ungrounded target; stable but harmful with task-grounded local CE). Neighborhood-local (CE flows through interface): YES at N=2, effect small but real (-0.006). **N=3 star topology is seed-sensitive** — rate-4 helper always rejected, seeds disagree on whether it helps or hurts. Real finding is instability (15× more variance than baseline). Currently testing G_rate4_only (rate-4 in isolation). Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
