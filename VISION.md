...
**Closed-loop hierarchical prediction — mechanism works, neighborhood-local confirmed as minimum viable locality.** Block 1 predicts block 0's future states and feeds predictions back via additive zero-init gate. V3 result (2 seeds): C < A by 0.006, gain spontaneously goes negative (predictive coding emerges). The full locality series is complete: strict-local with ungrounded target collapses (D); strict-local with task-grounded local CE is stable but doesn't help (E, +0.026); semi-local (CE through interface) helps (C, -0.006). **The feedback gradient does two jobs: prevents collapse AND teaches what to predict. Local CE can replace the first but not the second.** N=3 star topology (two helpers both predict block 0) is **seed-sensitive** — creates 15× more variance than baseline, one seed hurts (+0.034) one helps (-0.013). Rate-4 helper always rejected (gain → 0). **G_rate4_only confirmed: rate-4 intrinsically too stale (ablation gap = 0, pred_loss rises over training).** Currently testing phase offsets (I). See `research/questions/local-learning-variants/README.md`.

**Framing correction** (per [dictation 2026-05-24-5](dictations/2026-05-24-5.md)): These blocks are **feedforward, not RNNs.** Block 1 should predict "something block 0 couldn't already know" — e.g. information from a longer time ago — not block 0's current hidden state. Current experiments use full-state prediction as a simplified target for mechanism testing. The prediction target must change after mechanism questions are resolved. See `research/questions/local-learning-theory/README.md`.
...
- **Can local learning scale?** Partially answered. Strict-local: NO (collapses with ungrounded target; stable but harmful with task-grounded local CE). Neighborhood-local (CE flows through interface): YES at N=2, effect small but real (-0.006). N=3 star topology seed-sensitive; rate-4 intrinsically too stale (G confirmed ablation gap = 0). **Currently testing I (two rate-2 helpers at different phases, per-helper losses).** Interim result: I_phase_offset ≈ C — width doesn't increase benefit beyond -0.006. Hypothesis: prediction target is the bottleneck (block 0 already knows its own state). Phase 5 prediction target experiments designed (J/K/L). Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
