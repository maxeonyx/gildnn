...
- **Can local learning scale?** Partially answered. Strict-local: NO (collapses with ungrounded target; stable but harmful with task-grounded local CE). Neighborhood-local (CE flows through interface): YES at N=2, effect small but real (-0.006). **Currently testing N=3 star topology** — two helpers both predict block 0. If width composes, the path to "many more parallel blocks" is clear. Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
