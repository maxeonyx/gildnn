...
**Closed-loop hierarchical prediction — mechanism works, neighborhood-local confirmed as minimum viable locality.** Block 1 predicts block 0's future states and feeds predictions back via additive zero-init gate. V3 result (2 seeds): C < A by 0.006, gain spontaneously goes negative (predictive coding emerges). The full locality series is complete: strict-local with ungrounded target collapses (D); strict-local with task-grounded local CE is stable but doesn't help (E, +0.026); semi-local (CE through interface) helps (C, -0.006). **The feedback gradient does two jobs: prevents collapse AND teaches what to predict. Local CE can replace the first but not the second.** N=3 star topology (two helpers both predict block 0) is **seed-sensitive** — creates 15× more variance than baseline. Rate-4 helper always rejected (gain → 0). **G_rate4_only confirmed: rate-4 intrinsically too stale.** **I (phase offsets) confirmed: width saturates at -0.006 with full-state target. Target is the bottleneck.** Phase 5 (prediction target change) implemented — testing whether predicting "something block 0 doesn't know" breaks the saturation. See `research/questions/local-learning-variants/README.md`.

**Framing correction** (per [dictation 2026-05-24-5](dictations/2026-05-24-5.md)): These blocks are **feedforward, not RNNs.** Block 1 should predict "something block 0 couldn't already know" — e.g. information from a longer time ago — not block 0's current hidden state. Phases 1-4 used full-state prediction for mechanism testing; mechanism questions are now resolved. Phase 5 changes the target. See `research/questions/local-learning-theory/README.md`.
...
- **Can local learning scale?** Partially answered. Strict-local: NO (collapses with ungrounded target; stable but harmful with task-grounded local CE). Neighborhood-local (CE flows through interface): YES at N=2, effect small but real (-0.006). **Width saturates at -0.006 with full-state target** — I confirmed (two rate-2 helpers get same benefit as one). Per-helper losses fix N=3 coupling instability; rate-4 intrinsically too stale. **The binding constraint is the prediction target, not architecture.** Phase 5 (predict something block 0 doesn't know) implemented and ready to run. If Phase 5 breaks -0.006, the target was the bottleneck and scaling reopens. Max's framing: "It doesn't have to be totally local. We can be using backpropagation through a local neighborhood of blocks." See `research/questions/local-learning-variants/README.md`.
