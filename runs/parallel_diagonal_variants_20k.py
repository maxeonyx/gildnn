...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = (
        ...
        / "fixed_multi_rate"
        / "artifacts"
        / "parallel_diagonal_variants_20k"
    )
...
def build_parallel_last_step1_model(*, device: torch.device) -> ParallelDiagonalModel:
...
def build_parallel_all_step1_model(*, device: torch.device) -> ParallelDiagonalModel:
    return ParallelDiagonalModel(
        vocab_size=VOCAB_SIZE,
        context_size=CONTEXT_SIZE,
        d_model=PARALLEL_D_MODEL,
...
def build_parallel_last_step2_model(*, device: torch.device) -> ParallelDiagonalModel:
    return ParallelDiagonalModel(
        vocab_size=VOCAB_SIZE,
        context_size=CONTEXT_SIZE,
        d_model=PARALLEL_D_MODEL,
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        ...
        "parallel_diagonal_last_step1": VariantSpec(
            key="parallel_diagonal_last_step1",
            label="parallel_diagonal_4x128_ff256_last_step1",
        ...
        "parallel_diagonal_all_step1": VariantSpec(
            key="parallel_diagonal_all_step1",
            label="parallel_diagonal_4x128_ff256_all_step1",
            class_name="ParallelDiagonalModel",
            d_model=PARALLEL_D_MODEL,
            feedforward_dim=PARALLEL_FEEDFORWARD_DIM,
            internal_steps=1,
        ...
        "parallel_diagonal_last_step2": VariantSpec(
            key="parallel_diagonal_last_step2",
            label="parallel_diagonal_4x128_ff256_last_step2",
            class_name="ParallelDiagonalModel",
            d_model=PARALLEL_D_MODEL,
            feedforward_dim=PARALLEL_FEEDFORWARD_DIM,
            internal_steps=2,
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/parallel_diagonal_variants_20k.py.")
    ...
    builders = {
        ...
        "parallel_diagonal_last_step1": build_parallel_last_step1_model,
        "parallel_diagonal_all_step1": build_parallel_all_step1_model,
        "parallel_diagonal_last_step2": build_parallel_last_step2_model,
    }
    specs = variant_specs()
    ...
    report = {
        ...
        "comparison": {
            "delta_control_minus_parallel_diagonal_last_step1": {
                "final_val_loss": round(
                    report_models["control"]["final_checkpoint"]["val_loss"]
                    - report_models["parallel_diagonal_last_step1"]["final_checkpoint"]["val_loss"],
                    6,
                ),
                "final_val_accuracy": round(
                    report_models["control"]["final_checkpoint"]["val_accuracy"]
                    - report_models["parallel_diagonal_last_step1"]["final_checkpoint"]["val_accuracy"],
                    6,
                ),
            },
            "delta_control_minus_parallel_diagonal_all_step1": {
                "final_val_loss": round(
                    report_models["control"]["final_checkpoint"]["val_loss"]
                    - report_models["parallel_diagonal_all_step1"]["final_checkpoint"]["val_loss"],
                    6,
                ),
                "final_val_accuracy": round(
                    report_models["control"]["final_checkpoint"]["val_accuracy"]
                    - report_models["parallel_diagonal_all_step1"]["final_checkpoint"]["val_accuracy"],
                    6,
                ),
            },
            "delta_control_minus_parallel_diagonal_last_step2": {
                "final_val_loss": round(
                    report_models["control"]["final_checkpoint"]["val_loss"]
                    - report_models["parallel_diagonal_last_step2"]["final_checkpoint"]["val_loss"],
                    6,
                ),
                "final_val_accuracy": round(
                    report_models["control"]["final_checkpoint"]["val_accuracy"]
                    - report_models["parallel_diagonal_last_step2"]["final_checkpoint"]["val_accuracy"],
                    6,
                ),
            },
            "delta_parallel_diagonal_all_step1_minus_parallel_diagonal_last_step1": {
                "final_val_loss": round(
                    report_models["parallel_diagonal_all_step1"]["final_checkpoint"]["val_loss"]
                    - report_models["parallel_diagonal_last_step1"]["final_checkpoint"]["val_loss"],
                    6,
            ...
            "delta_parallel_diagonal_last_step2_minus_parallel_diagonal_last_step1": {
                "final_val_loss": round(
                    report_models["parallel_diagonal_last_step2"]["final_checkpoint"]["val_loss"]
                    - report_models["parallel_diagonal_last_step1"]["final_checkpoint"]["val_loss"],
                    6,
