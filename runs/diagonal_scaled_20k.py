...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-path",
        type=Path,
        default=repo_root / "experiments" / "fixed_multi_rate" / "artifacts" / "diagonal_scaled_20k" / "report.json",
    )
    return parser.parse_args()


def build_model(*, device: torch.device, diagonal_mode: str) -> MultiRateResidualModel:
...
def log_checkpoint(*, step: int, control_loss: float, diagonal_loss: float) -> None:
    print(
        f"Step {step}: control val_loss={control_loss:.6f} diagonal_scaled val_loss={diagonal_loss:.6f}",
        flush=True,
    )


def diagonal_alpha_values(model: MultiRateResidualModel) -> list[float]:
    return [round(scale.detach().item(), 6) for scale in model.diagonal_scales]
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/diagonal_scaled_20k.py.")
    ...
    set_seed(args.seed)
    control_model = build_model(device=device, diagonal_mode="off")
    ...
    set_seed(args.seed)
    diagonal_model = build_model(device=device, diagonal_mode="scaled")
    ...
    for zero_based_index, indices in enumerate(batch_schedule[WARMUP_STEPS:], start=WARMUP_STEPS):
        ...
        if not diagonal_diverged:
            ...
            if diagonal_checkpoint["val_loss"] > DIVERGENCE_THRESHOLD:
                ...
                print(
                    f"DIVERGENCE: diagonal_scaled val_loss={diagonal_checkpoint['val_loss']:.6f} exceeded {DIVERGENCE_THRESHOLD:.1f} at step {step}",
                    flush=True,
                )
        else:
    ...
    final_diagonal_alphas = diagonal_alpha_values(diagonal_model)
    ...
    report = {
        ...
        "diverged_models": [
            model_name
            for model_name, diverged in (("control", control_diverged), ("diagonal_scaled", diagonal_diverged))
            if diverged
        ],
        "models": {
            "control": {
            ...
            "diagonal_scaled": {
                "label": "multi_rate_4x128_1248_diagonal_scaled",
                "d_model": 128,
                "feedforward_dim": 256,
                "rates": [1, 2, 4, 8],
                "diagonal_mode": "scaled",
                ...
                "final_diagonal_alphas": final_diagonal_alphas,
            },
        },
        "delta_control_minus_diagonal_scaled": {
    ...
    print(f"Final diagonal alphas: {final_diagonal_alphas}", flush=True)
