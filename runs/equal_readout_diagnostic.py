...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = repo_root / "experiments" / "wikitext_103" / "artifacts" / "equal_readout"
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        ...
        "B_learned_mix": VariantSpec(
            key="B_learned_mix",
            label="equal_readout_B_learned_mix",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
            rates=(1, 2, 4, 8),
        ...
        "C_equal_mix": VariantSpec(
            key="C_equal_mix",
            label="equal_readout_C_equal_mix",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
            rates=(1, 2, 4, 8),
...
def build_model(
    *,
    ...
    effective_readout_mode = "all" if spec.readout_mode == "equal" else spec.readout_mode
    ...
    if spec.readout_mode == "equal":
        if model.readout_logits is None:
            raise RuntimeError("Equal readout requires readout_logits to exist.")
        with torch.no_grad():
            model.readout_logits.zero_()
...
def comparison(summary_by_variant: dict[str, object]) -> dict[str, float]:
    return {
        "mean_final_val_loss_delta_B_minus_A": round(
            summary_by_variant["B_learned_mix"]["mean_final_val_loss"]
            - summary_by_variant["A_single"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_B_minus_A": round(
            summary_by_variant["B_learned_mix"]["mean_final_val_accuracy"]
            - summary_by_variant["A_single"]["mean_final_val_accuracy"],
            6,
        ),
        "mean_final_val_loss_delta_C_minus_A": round(
            summary_by_variant["C_equal_mix"]["mean_final_val_loss"]
            - summary_by_variant["A_single"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_C_minus_A": round(
            summary_by_variant["C_equal_mix"]["mean_final_val_accuracy"]
            - summary_by_variant["A_single"]["mean_final_val_accuracy"],
            6,
        ),
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/equal_readout_diagnostic.py.")
