...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = (
        ...
        / "fixed_multi_rate"
        / "artifacts"
        / "parallel_diagonal_multirate_3seed"
    )
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/parallel_diagonal_multirate_3seed.py.")
    ...
    for seed in args.seeds:
        ...
        parallel_result = train_single_variant(
            seed=seed,
            variant_key="parallel_diagonal_multirate",
            spec=specs["parallel_diagonal_multirate"],
            builder=builders["parallel_diagonal_multirate"],
            args=args,
            device=device,
            train_inputs=train_inputs,
            train_targets=train_targets,
        ...
        seed_result = {
            ...
            "parallel_diagonal_multirate": parallel_result,
            "delta_control_minus_parallel_diagonal_multirate": delta,
        }
    ...
    averages = {
        ...
        "parallel_diagonal_multirate": {
            "final_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_multirate"]["final_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["parallel_diagonal_multirate"]["final_checkpoint"]["val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_multirate"]["best_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_training_loss": mean_rounded(
                [result["parallel_diagonal_multirate"]["final_training_loss"] for result in seed_results]
            ),
            "wall_seconds": mean_rounded(
                [result["parallel_diagonal_multirate"]["wall_seconds"] for result in seed_results]
            ),
        },
        "delta_control_minus_parallel_diagonal_multirate": {
            "final_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_multirate"]["final_val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_multirate"]["final_val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_multirate"]["best_val_loss"]
                    for result in seed_results
                ]
            ),
        },
    ...
    append_log(
        args.log_path,
        {
            ...
            "average_delta_control_minus_parallel_diagonal_multirate": averages[
                "delta_control_minus_parallel_diagonal_multirate"
            ],
        },
    )
