...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = repo_root / "experiments" / "wikitext_103" / "artifacts" / "ctx128_corrected"
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        "A_single": VariantSpec(
            key="A_single",
            label="ctx128_corrected_A_single",
            d_model=D_MODEL,
        ...
        "B_corrected": VariantSpec(
            key="B_corrected",
            label="ctx128_corrected_B_corrected",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/ctx128_corrected.py.")
