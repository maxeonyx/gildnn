...
if __package__ in (None, ""):
    raise RuntimeError(
        "Run from the repo root with `.\\.venv\\Scripts\\python.exe -m runs.stream_combining` so `core` imports resolve cleanly."
    )
...
DEFAULT_BLOCK2_STEPS = 800
SANITY_BLOCK0_STEPS = 30
SANITY_BLOCK1_STEPS = 60
SANITY_BLOCK2_STEPS = 60
...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = repo_root / "experiments" / "tinyshakespeare" / "artifacts" / "stream_combining"
    ...
    parser.add_argument("--block2-steps", type=positive_int, default=DEFAULT_BLOCK2_STEPS)
    ...
    parser.add_argument("--block2-lr", type=positive_float, default=DEFAULT_BLOCK2_LR)
    parser.add_argument("--lambda-weight", type=non_negative_float, default=DEFAULT_LAMBDA_WEIGHT)
...
def resolve_block2_steps(args: argparse.Namespace) -> int:
    return SANITY_BLOCK2_STEPS if args.sanity_check_only else args.block2_steps
...
def resolve_block2_lr(args: argparse.Namespace) -> float:
    return SANITY_BLOCK2_LR if args.sanity_check_only else args.block2_lr
...
def resolve_block2_eval_interval(args: argparse.Namespace) -> int:
    return SANITY_BLOCK2_EVAL_INTERVAL if args.sanity_check_only else DEFAULT_BLOCK2_EVAL_INTERVAL
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if not math.isfinite(args.lambda_weight):
        errors.append(f"lambda_weight must be finite, got {args.lambda_weight}")
    if errors:
...
def prepare_tinyshakespeare_data(*, repo_root: Path, seq_len: int) -> TinyShakespeareData:
    ...
    with tempfile.TemporaryDirectory(prefix="gildnn_stream_combining_corpus_") as tmp_dir:
...
def predictor_loss_from_predictions(
    ...
    *,
    ...
    if valid_predictions.shape[1] == 0:
        raise ValueError("Need at least one valid position for predictor MSE loss.")
...
def predict_sequence_from_previous_actuals(
    predictor: RepresentationPredictorBlock,
    *,
    target_sequence: Float[Tensor, "batch seq d_model"],
    bptt_chunk: int,
    use_recurrence: bool,
) -> Float[Tensor, "batch seq d_model"]:
    hidden: Tensor | None = None
    previous_target = torch.zeros(
    ...
    for start, stop in chunk_ranges(target_sequence.shape[1], bptt_chunk):
        target_chunk = target_sequence[:, start:stop, :]
        predictor_input = torch.cat([previous_target.unsqueeze(1), target_chunk[:, :-1, :]], dim=1)
        predicted_chunk, hidden = predictor.forward_chunk(predictor_input, hidden, use_recurrence=use_recurrence)
        predicted_chunks.append(predicted_chunk)
        previous_target = target_chunk[:, -1, :].detach()
        hidden = hidden.detach()
    return torch.cat(predicted_chunks, dim=1)


def combine_streams(
    stream_mu: Float[Tensor, "batch seq d_model"],
    prediction_mu: Float[Tensor, "batch seq d_model"],
    *,
    lambda_weight: float,
) -> Float[Tensor, "batch seq d_model"]:
    if stream_mu.shape != prediction_mu.shape:
        raise ValueError(f"stream/prediction shape mismatch: {stream_mu.shape} vs {prediction_mu.shape}")
    ...
    combined = (stream_mu + lambda_weight * prediction_mu) / (1.0 + lambda_weight)
    return F.normalize(combined.float(), dim=-1).to(stream_mu.dtype)
...
def build_combined_sequence(
    ...
    *,
    ...
    block1_prediction = predict_sequence_from_previous_actuals(
        block1,
        target_sequence=block0_sequence,
...
@torch.inference_mode()
def evaluate_predictor_metrics(
    predictor: RepresentationPredictorBlock,
    *,
    target_sequence: Float[Tensor, "batch seq d_model"],
    bptt_chunk: int,
    ...
    predictor.eval()
    predicted_mu = predict_sequence_from_previous_actuals(
        predictor,
        target_sequence=target_sequence,
        bptt_chunk=bptt_chunk,
        use_recurrence=use_recurrence,
    )
    valid_target = target_sequence[:, 1:, :]
    valid_prediction = predicted_mu[:, 1:, :]
    copy_baseline = target_sequence[:, :-1, :]
    random_vector = random_baseline_vector(
        d_model=target_sequence.shape[2],
        device=target_sequence.device,
        dtype=target_sequence.dtype,
    )
...
def valid_target_chunk_count(*, seq_len: int, bptt_chunk: int) -> int:
    total_chunks = 0
    for start, stop in chunk_ranges(seq_len, bptt_chunk):
        chunk_length = stop - start
        valid_length = chunk_length - 1 if start == 0 else chunk_length
        if valid_length > 0:
            total_chunks += 1
    if total_chunks == 0:
        raise RuntimeError("Predictor training requires at least one valid target chunk.")
    return total_chunks
...
def train_predictor(
    ...
    *,
    train_sequences: Tensor,
    eval_target_sequence: Float[Tensor, "batch seq d_model"],
    build_target_sequence: callable,
    steps: int,
    batch_size: int,
    bptt_chunk: int,
    device: torch.device,
    ...
    use_recurrence: bool,
) -> tuple[PhaseSummary, PredictorMetrics]:
    rng = torch.Generator(device="cpu")
    rng.manual_seed(seed)
    initial_metrics = evaluate_predictor_metrics(
        predictor,
        target_sequence=eval_target_sequence,
        bptt_chunk=bptt_chunk,
        use_recurrence=use_recurrence,
    )
    checkpoints: list[PhaseCheckpoint] = [
    ...
    total_chunks = valid_target_chunk_count(seq_len=eval_target_sequence.shape[1], bptt_chunk=bptt_chunk)
    ...
    for step in range(1, steps + 1):
        ...
        with torch.no_grad():
            target_sequence = build_target_sequence(batch_tokens)

        optimizer.zero_grad(set_to_none=True)
        hidden: Tensor | None = None
        previous_target = torch.zeros(target_sequence.shape[0], target_sequence.shape[2], device=device, dtype=target_sequence.dtype)
        loss_sum = 0.0
        valid_positions = 0
        for start, stop in chunk_ranges(target_sequence.shape[1], bptt_chunk):
            target_chunk = target_sequence[:, start:stop, :]
            predictor_input = torch.cat([previous_target.unsqueeze(1), target_chunk[:, :-1, :]], dim=1)
            predicted_chunk, hidden = predictor.forward_chunk(predictor_input, hidden, use_recurrence=use_recurrence)
            chunk_loss, chunk_valid_positions = predictor_loss_from_predictions(
                predicted_chunk,
                target_chunk,
                skip_first_position=start == 0,
            )
            if not torch.isfinite(chunk_loss):
                raise RuntimeError(f"Predictor training diverged at step {step} for {checkpoint_stage}.")
            (chunk_loss / total_chunks).backward()
            loss_sum += chunk_loss.detach().item() * chunk_valid_positions
            valid_positions += chunk_valid_positions
            previous_target = target_chunk[:, -1, :].detach()
        ...
        torch.nn.utils.clip_grad_norm_(trainable_parameters(predictor), grad_clip_norm)
        optimizer.step()
        final_train_loss = loss_sum / valid_positions
        ...
        final_eval_metrics = evaluate_predictor_metrics(
            predictor,
            target_sequence=eval_target_sequence,
            bptt_chunk=bptt_chunk,
            use_recurrence=use_recurrence,
...
def main() -> int:
    ...
    block2_steps = resolve_block2_steps(args)
    ...
    register_active_lock(
        experiment_name="stream_combining",
        variants=[f"dm{args.d_model}", f"seq{seq_len}", f"chunk{args.bptt_chunk}", f"lambda{args.lambda_weight}"],
    ...
    block2_combined_optimizer = torch.optim.AdamW(
        trainable_parameters(block2_combined),
        lr=block2_lr,
        betas=(0.9, 0.999),
        weight_decay=DEFAULT_WEIGHT_DECAY,
        capturable=device.type == "cuda",
    ...
    phase_started_at = perf_counter()
    phase_b_eval_target = collect_block0_representations(block0, eval_batch_tokens, bptt_chunk=args.bptt_chunk)
    phase_b_summary, phase_b_metrics = train_predictor(
        block1,
        block1_optimizer,
        train_sequences=train_sequences,
        eval_target_sequence=phase_b_eval_target,
        build_target_sequence=lambda batch_tokens: collect_block0_representations(
            block0,
            batch_tokens,
            bptt_chunk=args.bptt_chunk,
        ),
    ...
    for parameter in block1.parameters():
        parameter.requires_grad_(False)

    with torch.no_grad():
        phase_c_passthrough_eval_target = collect_block0_representations(block0, eval_batch_tokens, bptt_chunk=args.bptt_chunk)
        phase_c_combined_eval_target = build_combined_sequence(
            block1,
            block0_sequence=phase_c_passthrough_eval_target,
            bptt_chunk=args.bptt_chunk,
            lambda_weight=args.lambda_weight,
            use_recurrence=True,
        )
    ...
    phase_c_combined_summary, phase_c_combined_metrics = train_predictor(
        block2_combined,
        block2_combined_optimizer,
        train_sequences=train_sequences,
        eval_target_sequence=phase_c_combined_eval_target,
        build_target_sequence=lambda batch_tokens: build_combined_sequence(
            block1,
            block0_sequence=collect_block0_representations(block0, batch_tokens, bptt_chunk=args.bptt_chunk),
            bptt_chunk=args.bptt_chunk,
            lambda_weight=args.lambda_weight,
            use_recurrence=True,
        ),
        ...
        seed=args.seed + 2,
        log_path=args.log_path,
        checkpoint_stage="phase_c_combined_checkpoint",
        use_recurrence=True,
    )
    phase_c_combined_wall_seconds = round(perf_counter() - phase_started_at, 6)

    phase_started_at = perf_counter()
    phase_c_passthrough_summary, phase_c_passthrough_metrics = train_predictor(
        block2_passthrough,
        block2_passthrough_optimizer,
        train_sequences=train_sequences,
        eval_target_sequence=phase_c_passthrough_eval_target,
        build_target_sequence=lambda batch_tokens: collect_block0_representations(
            block0,
            batch_tokens,
            bptt_chunk=args.bptt_chunk,
        ...
        seed=args.seed + 2,
        log_path=args.log_path,
        checkpoint_stage="phase_c_passthrough_checkpoint",
        use_recurrence=True,
    )
    ...
    summary_lines = [
        ...
        f"  Phase B copy baseline MSE: {phase_b_metrics.copy_baseline:.6f}",
        f"  Phase C combined MSE: {phase_c_combined_metrics.mse:.6f}",
    ...
    report = {
        ...
        "results": {
            ...
            "phase_c_combined": {
                "initial_eval_mse": phase_c_combined_summary.initial_eval_loss,
                "final_train_mse": phase_c_combined_summary.final_train_loss,
                "final_eval_mse": phase_c_combined_metrics.mse,
                "copy_baseline": phase_c_combined_metrics.copy_baseline,
                "random_baseline": phase_c_combined_metrics.random_baseline,
                "beats_copy": phase_c_combined_metrics.beats_copy,
                "loss_went_down": phase_c_combined_metrics.mse < phase_c_combined_summary.initial_eval_loss,
                "wall_seconds": phase_c_combined_wall_seconds,
                "checkpoints": phase_checkpoints_to_payload(phase_c_combined_summary.checkpoints),
            },
            "phase_c_passthrough": {
                "initial_eval_mse": phase_c_passthrough_summary.initial_eval_loss,
                "final_train_mse": phase_c_passthrough_summary.final_train_loss,
                "final_eval_mse": phase_c_passthrough_metrics.mse,
                "copy_baseline": phase_c_passthrough_metrics.copy_baseline,
                "random_baseline": phase_c_passthrough_metrics.random_baseline,
                "beats_copy": phase_c_passthrough_metrics.beats_copy,
                "loss_went_down": phase_c_passthrough_metrics.mse < phase_c_passthrough_summary.initial_eval_loss,
    ...
    append_log(
        args.log_path,
        {
            ...
            "phase_c_combined_final_eval_mse": round(phase_c_combined_metrics.mse, 6),
