...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = repo_root / "experiments" / "fixed_multi_rate" / "artifacts" / "bidirectional_sanity"
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        "A_upward_all": VariantSpec(
            key="A_upward_all",
            label="bidirectional_sanity_A_upward_all",
            d_model=D_MODEL,
        ...
        "B_topdown_first": VariantSpec(
            key="B_topdown_first",
            label="bidirectional_sanity_B_topdown_first",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
            rates=FOUR_BLOCK_RATES,
        ...
        "C_topdown_all": VariantSpec(
            key="C_topdown_all",
            label="bidirectional_sanity_C_topdown_all",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
            rates=FOUR_BLOCK_RATES,
        ...
        "D_single_block": VariantSpec(
            key="D_single_block",
            label="bidirectional_sanity_D_single_block",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=1,
            rates=(1,),
...
def comparison(summary_by_variant: dict[str, object]) -> dict[str, float]:
    return {
        "mean_final_val_loss_delta_B_minus_D": round(
            summary_by_variant["B_topdown_first"]["mean_final_val_loss"]
            - summary_by_variant["D_single_block"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_B_minus_D": round(
            summary_by_variant["B_topdown_first"]["mean_final_val_accuracy"]
            - summary_by_variant["D_single_block"]["mean_final_val_accuracy"],
            6,
        ),
        "mean_final_val_loss_delta_C_minus_D": round(
            summary_by_variant["C_topdown_all"]["mean_final_val_loss"]
            - summary_by_variant["D_single_block"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_C_minus_D": round(
            summary_by_variant["C_topdown_all"]["mean_final_val_accuracy"]
            - summary_by_variant["D_single_block"]["mean_final_val_accuracy"],
            6,
        ),
        "mean_final_val_loss_delta_B_minus_A": round(
            summary_by_variant["B_topdown_first"]["mean_final_val_loss"]
            - summary_by_variant["A_upward_all"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_B_minus_A": round(
            summary_by_variant["B_topdown_first"]["mean_final_val_accuracy"]
            - summary_by_variant["A_upward_all"]["mean_final_val_accuracy"],
            6,
        ),
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/bidirectional_sanity.py.")
