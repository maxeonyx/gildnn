...
class ContextAsymmetryModel(nn.Module):
    ...
    def forward(
        ...
        *,
        use_lateral: bool,
        shuffle_lateral: bool,
    ) -> tuple[Float[Tensor, "batch vocab"], Float[Tensor, "batch long_context vocab"]]:
        ...
        recon_logits = self.recon_head(h1)
...
def compute_losses(
    ...
    *,
    ...
    if condition.use_local_loss:
        local_loss = F.cross_entropy(recon_logits.view(-1, model.vocab_size), inputs.view(-1))
...
def main() -> None:
    ...
    print(
        "context-asymmetry reconstruction test "
