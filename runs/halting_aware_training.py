...
class RecurrentDepthLM(nn.Module):
    ...
    def halt_logits_from_hidden(self, last_hidden: Tensor, *, depth_index: int) -> Tensor:
...
def compute_depth_outputs(
    ...
    for depth_index, iteration_hidden in enumerate(iteration_states, start=1):
        ...
        if depth_index < model.config.recurrent_iterations:
            halt_logits.append(model.halt_logits_from_hidden(last_hidden, depth_index=depth_index))
