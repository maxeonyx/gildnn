...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = repo_root / "experiments" / "wikitext_103" / "artifacts" / "temporal_window"
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        "A_no_window": VariantSpec(
            key="A_no_window",
            label="temporal_window_A_no_window",
            d_model=D_MODEL,
        ...
        "B_window_8": VariantSpec(
            key="B_window_8",
            label="temporal_window_B_window_8",
            d_model=D_MODEL,
            feedforward_dim=FEEDFORWARD_DIM,
            num_blocks=4,
            rates=(1, 2, 4, 8),
    ...
    *,
    ...
    for key, runs in grouped.items():
        ...
        summary[key] = {
            ...
            "temporal_window": specs[key].temporal_window,
            "num_runs": len(runs),
...
def comparison(summary_by_variant: dict[str, object]) -> dict[str, float]:
    return {
        "mean_final_val_loss_delta_B_minus_A": round(
            summary_by_variant["B_window_8"]["mean_final_val_loss"]
            - summary_by_variant["A_no_window"]["mean_final_val_loss"],
            6,
        ),
        "mean_final_val_accuracy_delta_B_minus_A": round(
            summary_by_variant["B_window_8"]["mean_final_val_accuracy"]
            - summary_by_variant["A_no_window"]["mean_final_val_accuracy"],
            6,
        ),
    }
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/temporal_window_diagnostic.py.")
