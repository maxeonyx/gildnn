...
WARMUP_STEPS = 3
NUM_BLOCKS = 4
CONTROL_RATES = (1, 1, 1, 1)
PARALLEL_RATES = (1, 2, 4, 8)
...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = (
        ...
        / "fixed_multi_rate"
        / "artifacts"
        / "parallel_diagonal_matched_flop_fixed_3seed"
    )
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        "control": VariantSpec(
            key="control",
            label="multi_rate_4x128_ff256_1111_control_fixed",
        ...
        "parallel_diagonal_matched_flop_fixed": VariantSpec(
            key="parallel_diagonal_matched_flop_fixed",
            label="parallel_diagonal_4x128_ff546_all_rate1248_matched_flop_fixed",
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/parallel_diagonal_matched_flop_fixed_3seed.py.")
    ...
    builders = {
        ...
        "parallel_diagonal_matched_flop_fixed": build_parallel_matched_flop_model,
    }
    ...
    for seed in args.seeds:
        ...
        parallel_result = train_single_variant(
            seed=seed,
            variant_key="parallel_diagonal_matched_flop_fixed",
            spec=specs["parallel_diagonal_matched_flop_fixed"],
            builder=builders["parallel_diagonal_matched_flop_fixed"],
            args=args,
            device=device,
            train_inputs=train_inputs,
            train_targets=train_targets,
        ...
        seed_result = {
            ...
            "parallel_diagonal_matched_flop_fixed": parallel_result,
            "delta_control_minus_parallel_diagonal_matched_flop_fixed": delta,
        }
    ...
    averages = {
        ...
        "parallel_diagonal_matched_flop_fixed": {
            "final_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop_fixed"]["final_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop_fixed"]["final_checkpoint"]["val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop_fixed"]["best_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_training_loss": mean_rounded(
                [result["parallel_diagonal_matched_flop_fixed"]["final_training_loss"] for result in seed_results]
            ),
            "wall_seconds": mean_rounded(
                [result["parallel_diagonal_matched_flop_fixed"]["wall_seconds"] for result in seed_results]
            ),
        },
        "delta_control_minus_parallel_diagonal_matched_flop_fixed": {
            "final_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop_fixed"]["final_val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop_fixed"]["final_val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop_fixed"]["best_val_loss"]
                    for result in seed_results
                ]
            ),
            "parameter_count": mean_rounded(
                [
                    float(result["delta_control_minus_parallel_diagonal_matched_flop_fixed"]["parameter_count"])
                    for result in seed_results
                ]
            ),
        },
    ...
    append_log(
        args.log_path,
        {
            ...
            "average_delta_control_minus_parallel_diagonal_matched_flop_fixed": averages[
                "delta_control_minus_parallel_diagonal_matched_flop_fixed"
            ],
        },
    )
