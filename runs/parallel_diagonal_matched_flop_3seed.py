...
def parse_args() -> argparse.Namespace:
    ...
    artifact_dir = (
        ...
        / "fixed_multi_rate"
        / "artifacts"
        / "parallel_diagonal_matched_flop_3seed"
    )
...
def variant_specs() -> dict[str, VariantSpec]:
    return {
        ...
        "parallel_diagonal_matched_flop": VariantSpec(
            key="parallel_diagonal_matched_flop",
            label="parallel_diagonal_4x128_ff546_all_rate1248",
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for runs/parallel_diagonal_matched_flop_3seed.py.")
    ...
    builders = {
        ...
        "parallel_diagonal_matched_flop": build_parallel_matched_flop_model,
    }
    ...
    append_log(
        args.log_path,
        {
            ...
            "trainer": "GraphTrainer",
            "matched_flop_design": {
                "control_average_block_evals_per_token": 4.0,
                "parallel_average_block_evals_per_token": 1.875,
                "control_per_token_block_flops": 4 * 2 * CONTROL_D_MODEL * CONTROL_FEEDFORWARD_DIM,
                "parallel_actual_per_token_block_flops": per_token_block_flops(
                    d_model=PARALLEL_D_MODEL,
                    feedforward_dim=PARALLEL_FEEDFORWARD_DIM,
                    average_block_evals_per_token=1.875,
                ),
            },
        },
    )
    ...
    for seed in args.seeds:
        ...
        parallel_result = train_single_variant(
            seed=seed,
            variant_key="parallel_diagonal_matched_flop",
            spec=specs["parallel_diagonal_matched_flop"],
            builder=builders["parallel_diagonal_matched_flop"],
            args=args,
            device=device,
            train_inputs=train_inputs,
            train_targets=train_targets,
        ...
        seed_result = {
            ...
            "parallel_diagonal_matched_flop": parallel_result,
            "delta_control_minus_parallel_diagonal_matched_flop": delta,
        }
    ...
    averages = {
        ...
        "parallel_diagonal_matched_flop": {
            "final_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop"]["final_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop"]["final_checkpoint"]["val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["parallel_diagonal_matched_flop"]["best_checkpoint"]["val_loss"]
                    for result in seed_results
                ]
            ),
            "final_training_loss": mean_rounded(
                [result["parallel_diagonal_matched_flop"]["final_training_loss"] for result in seed_results]
            ),
            "wall_seconds": mean_rounded(
                [result["parallel_diagonal_matched_flop"]["wall_seconds"] for result in seed_results]
            ),
        },
        "delta_control_minus_parallel_diagonal_matched_flop": {
            "final_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop"]["final_val_loss"]
                    for result in seed_results
                ]
            ),
            "final_val_accuracy": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop"]["final_val_accuracy"]
                    for result in seed_results
                ]
            ),
            "best_val_loss": mean_rounded(
                [
                    result["delta_control_minus_parallel_diagonal_matched_flop"]["best_val_loss"]
                    for result in seed_results
                ]
            ),
            "parameter_count": mean_rounded(
                [
                    float(result["delta_control_minus_parallel_diagonal_matched_flop"]["parameter_count"])
                    for result in seed_results
                ]
            ),
        },
    ...
    report = {
        ...
        "matched_flop_design": {
            ...
            "parallel_average_block_evals_per_token": 1.875,
            "control_per_token_block_flops": 4 * 2 * CONTROL_D_MODEL * CONTROL_FEEDFORWARD_DIM,
            "parallel_per_token_block_flops": round(
                1.875 * 2 * PARALLEL_D_MODEL * PARALLEL_FEEDFORWARD_DIM
            ),
            "parallel_block_flop_delta_vs_control": round(
                (1.875 * 2 * PARALLEL_D_MODEL * PARALLEL_FEEDFORWARD_DIM)
                - (4 * 2 * CONTROL_D_MODEL * CONTROL_FEEDFORWARD_DIM)
            ),
        },
    ...
    append_log(
        args.log_path,
        {
            ...
            "average_delta_control_minus_parallel_diagonal_matched_flop": averages[
                "delta_control_minus_parallel_diagonal_matched_flop"
            ],
        },
    )
