...
Next steps:
- [ ] **Hierarchical prediction experiment** — RUNNING (PID 7252, ~35 min). Tests whether slow blocks predicting future fast-block bottleneck latents improves task loss. 3 conditions × 3 seeds. See `experiments/fixed_multi_rate/artifacts/hierarchical_prediction/`.
- [ ] **LOCAL LEARNING CONFIRMED — integrate further** — Lateral gradient terms are noise (avg -0.009 = detaching is better). `detach_lateral` flag added to `ParallelDiagonalModel` (commit b570129). Next steps: (a) make it the default for new experiments, (b) measure actual wall-clock speedup from smaller computation graph.
