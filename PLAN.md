...
1. **Mix-Add vs plain residual** — now that the tiny fixed-window PyTorch reference stack has been integrated into `core/` and the tracked post-integration provenance is refreshed consistently, the first concrete post-integration bounded unit is the Mix-Add implementation and comparison against plain residual. Keep this narrow and artifact-backed.
